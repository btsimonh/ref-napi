...
types.int64 = {
  ...
  set: function set (buf, offset, val) {
    if (val.type && val.type.name === 'int64'){
      val.copy(buf, offset || 0, 0, 8);
      return;      
    }
...
types.uint64 = {
  ...
  set: function set (buf, offset, val) {
    if (val.type && val.type.name === 'uint64'){
      val.copy(buf, offset || 0, 0, 8);
      return;      
    }
...
Buffer.prototype.toString = overwriteToString (Buffer.prototype.toString);
 ...
 */
  ...
  SlowBuffer.prototype.toString = overwriteToString (SlowBuffer.prototype.toString);
}
...
function overwriteInspect (inspect) {
  if (inspect.name === 'refinspect') {
    ...
    return function refinspect () {
      if (this.type && this.type.size === 0){
        return `<Buffer@0x${this.hexAddress()} - void* size zero>`;
      }
...
function overwriteToString (toString) {
  if (toString.name === 'reftoString') {
    return toString;
  } else {
    return function reftoString () {
      if (this.type && this.type.size === 0){
        return `<void* size zero>`;
      }
      var v = toString.apply(this, arguments);
      return v;
    }
  }
}
